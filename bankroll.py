...
def calibrate_ml(prob):
    """Piecewise calibration. XGBoost overestimates high probabilities.
    70% model → 65% actual, 80% model → 72% actual."""
    if prob < 0.50:
    ...
    return min(0.725 + (prob - 0.80) * 0.25, 0.85)  # cap at 85%
...
def calibrate_ou(prob):
    """Piecewise O/U calibration. Model overconfident above 70%.
    70% model → 56% actual, 80% model → 71% actual."""
    if prob < 0.50:
